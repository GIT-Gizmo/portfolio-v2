    ...
    return (
        <section className="w-full py-20">
            ...
            <div className="my-20 flex flex-col lg:flex-row items-center justify-center w-full gap-4">
                {approach.map(({ id, phase, title, description, color, containerClassName, animationSpeed }) => (
                    <Card
                      key={id}
                      title={title}
                      description={description}
                      icon={<ShimmerButton order={phase} />}
                    >
                    <CanvasRevealEffect
...
const ShimmerButton = ({ order}: {order: string}) => {
    return (
        <button className="inline-flex h-12 animate-shimmer items-center justify-center rounded-full border border-slate-800 bg-[linear-gradient(110deg,#020513,45%,#040A21,55%,#020513)] bg-[length:200%_100%] px-6 py-2 font-bold text-purple transition-colors text-2xl">
          {order}
        </button>
    );
};
